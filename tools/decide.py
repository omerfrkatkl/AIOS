...
The owner approves decisions but does not write them, and prose dictated in chat
had to be rewritten by hand before landing - two steps and a rewrite, three
chances to drift. Structured fields go straight in.

Usage:
  python tools/decide.py --tier T-B --title "..." --decision "..." --reason "..." \\
      [--alternatives "A (elendi: ...)|B (elendi: ...)"] \\
      [--reversal "..."] [--evidence gözlendi|üretildi|varsayıldı] \\
      [--pending] [--closes "2026-08-15/Exact earlier title"]

  --pending  marks the entry as awaiting the owner's approval (T-A default)
  --closes   supersession link; repeatable, closes an earlier entry
"""
...
# The tag written into the document is Turkish, but a CLI token with
# diacritics is hostile to typing and pasting on Windows. Accept ASCII and
# English aliases, write the canonical form. Tolerant input, canonical output.
EVIDENCE = {
...
def main() -> int:
    ...
    print("Run `python tools/review.py` to confirm.")
    return 0
