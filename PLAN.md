...
Yöntem seçimi (G17) · araştırma hattı (soru→yöntem→kaynak→sentez→**kanıt-etiketli rapor**) · **araştırma önbelleği** (aynı soru → önce geçmiş rapor + tazelik kontrolü) · **puan girdileri raporlara atıfta zorunlu** · kaynak kütüğü · **araştırma planı formatı** (sahibin senaryosu: görev → kanal önerileri + kota notu ["Claude limiti bitiyor, yarım saat kaldı"] + çoklu-getiri ["üçünü birden yaptır"]) · **sindir.py + Tur 2 düzeltmesi:** interview sorularının araştırılabilir kısmı buraya taşındı (sahibin düzeltmesi) · sindir.py (web çıktısı yapıştırılır → LEDGER taraması + istek eşleşmesi + verdict; web AI'ın dosya erişimi olmadan doğrulayamadığını sistem telafi eder) · **provenance rozeti** (artefakt üstverisi: hangi model/kanal/raporlar/kanıt) · **ilk gerçek işler (sahibin düzeltmesi 2026-08-24):** model-benchmark araştırması (hangi model hangi işte iyi — sahibe SORULMAZ, buraya gelir) + araç karşılaştırmaları. **Test:** örnek soruda rapor üretilir; önbellek isabeti; sindir örnek çıktıda verdict.
