...
- [x] model kartı şeması (Kanal Sözleşmesi) · [x] OpenWebUI/LM Studio öncül araştırması · [x] üretici script (tools/registry.py: init/validate/list/route/update) · [ ] **senin envanter oturumun** · [ ] limit doğrulama araştırmaları · [x] gizlilik bölgesi (kart şemasında gizlilik alanı + route --gizli filtresi) · [x]–[ ] yönlendirici v1 (route yetenek-eşleşme + gerekçe; kota-takip kısmı F12c'ye) · [x] registry.py --update (sözlü bildirim stub — ajan akışı) · [ ] yetenek sağlayıcılar · [ ] bağımlılık grafiği · [ ] araç-yönlendirme · [x] test (tests/test_registry.py 7 senaryo · 20/20 toplam)
